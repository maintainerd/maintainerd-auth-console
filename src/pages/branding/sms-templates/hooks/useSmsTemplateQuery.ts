/**
 * SMS Template Query Hook
 * Manages query state for SMS templates listing
 */

import { useState, useMemo } from 'react'
import type { SortingState, PaginationState } from '@tanstack/react-table'
import { useSmsTemplates } from '@/hooks/useSmsTemplates'
import type { SmsTemplateStatusType } from '@/services/api/sms-template/types'

interface SmsTemplateFilters {
  status: SmsTemplateStatusType[]
}

export function useSmsTemplateQuery() {
  const [searchQuery, setSearchQuery] = useState('')
  const [filters, setFilters] = useState<SmsTemplateFilters>({
    status: [],
  })
  const [sorting, setSorting] = useState<SortingState>([
    { id: 'created_at', desc: true },
  ])
  const [pagination, setPagination] = useState<PaginationState>({
    pageIndex: 0,
    pageSize: 10,
  })

  // Build query params for API
  const queryParams = useMemo(() => {
    const params: Record<string, string | number> = {
      page: pagination.pageIndex + 1,
      limit: pagination.pageSize,
    }

    if (searchQuery) {
      params.name = searchQuery
    }

    if (filters.status.length > 0) {
      params.status = filters.status[0] // Backend expects single value
    }

    if (sorting.length > 0) {
      params.sort_by = sorting[0].id
      params.sort_order = sorting[0].desc ? 'desc' : 'asc'
    }

    return params
  }, [searchQuery, filters, sorting, pagination])

  // Fetch SMS templates
  const { data, isLoading, error } = useSmsTemplates(queryParams)

  const smsTemplates = data?.rows || []
  const rowCount = data?.total || 0

  return {
    smsTemplates,
    rowCount,
    isLoading,
    error,
    searchQuery,
    setSearchQuery,
    filters,
    setFilters,
    sorting,
    setSorting,
    pagination,
    setPagination,
  }
}
