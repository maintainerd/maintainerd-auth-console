...
export function SocialProviderActions({ provider }: SocialProviderActionsProps) {
  ...
  const handleViewDetails = () => {
    navigate(`/${tenantId}/providers/social/${provider.identity_provider_id}`)
  }
  ...
  const handleConfirmStatusChange = async () => {
    ...
    try {
      ...
      showSuccess(`Social provider status updated to ${pendingStatusAction.status}`)
    } catch (error) {
      showError(error)
    }
  }
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          {!isActive && (
            <DropdownMenuItem onClick={() => handleStatusChange("active", "Activate Social Provider", "Are you sure you want to activate this social provider?")} disabled={provider.is_system}>
          ...
          {!isInactive && (
            <DropdownMenuItem onClick={() => handleStatusChange("inactive", "Deactivate Social Provider", "Are you sure you want to deactivate this social provider?")} disabled={provider.is_system}>
      ...
      <ConfirmationDialog
        ...
        description={pendingStatusAction?.description || "Are you sure you want to change the social provider status?"}
        confirmText="Confirm"
        cancelText="Cancel"
        variant="default"
