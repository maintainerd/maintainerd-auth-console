...
import { useAddClientRole } from "@/hooks/useClients"
import { useToast } from "@/hooks/useToast"

interface AssignClientRolesDialogProps {
  open: boolean
...
export function AssignClientRolesDialog({
  open,
  onOpenChange,
  clientId,
  existingRoleIds,
}: AssignClientRolesDialogProps) {
  const [selectedRoles, setSelectedRoles] = useState<string[]>([])
  const [searchQuery, setSearchQuery] = useState("")

  const { showSuccess, showError } = useToast()
  const addRoleMutation = useAddClientRole(clientId)
  ...
  const handleSubmit = async (e?: FormEvent) => {
    ...
    try {
      await Promise.all(
        selectedRoles.map(roleId => addRoleMutation.mutateAsync(roleId))
      )
  ...
  const isLoading = addRoleMutation.isPending
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Assign Roles to Client</DialogTitle>
          <DialogDescription>
            Select roles to assign to this client. Already assigned roles are not shown.
          </DialogDescription>
        </DialogHeader>
